...
export async function restoreFromBackup(db: SQLiteDatabase): Promise<RestoreResult> {
  ...
  await db.withTransactionAsync(async () => {
    for (const table of RESTORE_ORDER) {
      ...
      // Clear existing data, then insert all backup rows
...
function validateBackup(data: Record<string, unknown[]>): void {
  const knownTables = new Set(['settings', 'accounts', 'categories', 'category_rules', 'import_batches', 'transactions', 'budgets']);
